...
if [[ ! -s x-ui-linux-${ARCH}.tar.gz ]]; then
    echo -e "${red}Скачивание 3x-ui не удалось${plain}" >&3
    exit 1
fi
...
if [[ -z "$XRAY_BIN" ]]; then
  echo -e "${red}Не найден бинарник xray${plain}" >&3
  exit 1
fi
chmod +x "$XRAY_BIN"
...
ADD_RESULT=$(curl -s -b "$COOKIE_JAR" -X POST "http://127.0.0.1:${PORT}/${WEBPATH}/panel/api/inbounds/add" \
  -H "Content-Type: application/json" \
  -d "{\"enable\":true,\"remark\":\"$remark\",\"listen\":\"0.0.0.0\",\"port\":443,\"protocol\":\"vless\"}")

if ! echo "$ADD_RESULT" | grep -q '"success":true'; then
    echo -e "${red}Ошибка при добавлении инбаунда.${plain}" >&3
    exit 1
fi

# Берём финальный VLESS прямо из 3x-ui
sleep 1
INBOUNDS_JSON=$(curl -s -b "$COOKIE_JAR" -X GET "http://127.0.0.1:${PORT}/${WEBPATH}/panel/api/inbounds/list")
VLESS_LINK=$(echo "$INBOUNDS_JSON" | jq -r '.data | sort_by(.id) | last | .shareLink')
rm -f "$COOKIE_JAR"

if [[ -z "$VLESS_LINK" || "$VLESS_LINK" == "null" ]]; then
    echo -e "${red}Не удалось получить VLESS из 3x-ui${plain}" >&3
    exit 1
fi

# Выводим и сохраняем
echo -e "\n\033[0;32mVLESS Reality успешно получен из панели!\033[0m" >&3
echo -e "\033[1;36mВаш VPN ключ:\033[0m" >&3
echo -e "${VLESS_LINK}" >&3
qrencode -t ANSIUTF8 "$VLESS_LINK"
qrencode -o /root/vless_qr.png "$VLESS_LINK"
echo -e "QR-код сохранён в файл: /root/vless_qr.png" >&3
{
  echo "Ваш VPN ключ:"
  echo "$VLESS_LINK"
  echo ""
